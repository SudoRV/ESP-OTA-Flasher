    ...
    case WStype_BIN:
      {
        if (!isUpdating) {
          ...
          Serial.println(info);
          String message = "{\"type\":\"info\", \"message\":\"" + info + "\"}";
          webSocket.sendTXT(clientID, message);
        }
        ...
        String message = "{\"type\":\"info\", \"message\":\"" + chunkInfo + "\"}";
        webSocket.sendTXT(clientID, message);
    ...
    case WStype_TEXT:
      {
        if (strcmp((const char*)payload, "END") == 0 && isUpdating) {
          ...
          if (Update.end(true)) {
            ...
            String message = "{\"type\":\"info\", \"status\":200, \"message\":\"" + success + "\"}";
            webSocket.sendTXT(clientID, message);
            webSocket.close();
    ...
    case WStype_DISCONNECTED:
      {
        if (isUpdating) {
          ...
          if (Update.end(true)) {
            ...
            Serial.println(success);
            String message = "{\"type\":\"info\", \"message\":\"" + success + "\"}";
            webSocket.sendTXT(clientID, message);
            delay(500);
            webSocket.close();
            ESP.restart();
